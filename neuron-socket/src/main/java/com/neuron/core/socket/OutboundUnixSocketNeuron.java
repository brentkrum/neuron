...
public class OutboundUnixSocketNeuron extends DefaultNeuronInstanceBase implements INeuronInitialization
{
	private static final Logger LOG = LogManager.getLogger(OutboundUnixSocketNeuron.class);
	...
	private final String m_unixSocketAddress;
	private final String m_statusAddress;
	...
	public OutboundUnixSocketNeuron(NeuronRef instanceRef, String socketAddress, long retryDelayInMS)
	{
		super(instanceRef);
		m_unixSocketAddress = socketAddress;
		m_statusAddress = m_unixSocketAddress;
	...
	public void init(Promise<Void> initPromise)
	{
		m_channelBootstrap.group(NeuronApplication.getIOPool())
			...
			.remoteAddress(new DomainSocketAddress(m_unixSocketAddress));
	
		// Need to only include this IF the log level is trace
		if (LOG.isTraceEnabled()) {
			m_channelBootstrap.handler(new LoggingHandler(OutboundUnixSocketNeuron.class, LogLevel.TRACE));
		}
		...
		try(INeuronStateLock lock = ref().lockState()) {
			lock.addStateAsyncListener(NeuronState.SystemOnline, (success, neuronRef, completedPromise) -> {
				if (success) {
					synchronized(OutboundUnixSocketNeuron.this) {
						m_connectionState = ConnectionState.Connecting;
					}
					NeuronApplication.logInfo(LOG, "Connecting to {}", m_statusAddress);
	...
	private class OutPipeReader implements IBytePipeBufReaderCallback {
		...
		public void onData(ByteBuf buf) {
			synchronized(OutboundUnixSocketNeuron.this) {
				if (m_connectionState != ConnectionState.Connected) {
					if (LOG.isTraceEnabled()) {
						LOG.trace("{} m_connectionState({}) != ConnectionState.Connected", m_statusAddress, m_connectionState);
					}
					return;
				}
				buf.retain();
				if (LOG.isTraceEnabled()) {
					LOG.trace("{} writeAndFlush", m_statusAddress);
				}
				...
				if (future.isSuccess()) {
					synchronized(OutboundUnixSocketNeuron.this) {
						...
						try(INeuronStateLock lock = ref().lockState()) {
							...
							StatusSystem.setHostStatus(m_statusAddress, StatusType.Up, "Connected");
						}
					...
					synchronized(OutboundUnixSocketNeuron.this) {
						...
						try(INeuronStateLock lock = ref().lockState()) {
							...
							if (takeOffline) {
								NeuronApplication.logError(LOG, "Connection to {} failed, taking neuron offline", m_statusAddress, cause);
								...
								if (m_repeatConnectError1 == 0) {
									NeuronApplication.logError(LOG, "Connection to {} failed, but will continue to retry every {} ms", m_statusAddress, m_retryDelayInMS, cause);
								}
								m_repeatConnectError1++;
							}
						}
						StatusSystem.setHostStatus(m_statusAddress, StatusType.Down, statusText + ". Retrying every " + m_retryDelayInMS + " ms.");
		...
		public void operationComplete(ChannelFuture future) throws Exception
		{
			synchronized(OutboundUnixSocketNeuron.this) {
				try(INeuronStateLock lock = ref().lockState()) {
					...
					if (m_deinitializing) {
						StatusSystem.setHostStatus(m_statusAddress, StatusType.Down, "Disconnected, neuron deinitialized");
					} else {
						StatusSystem.setHostStatus(m_statusAddress, StatusType.Down, "Disconnected unexpectedly, will continue to retry");
						// send a message in the pipe alerting to the disconnected state?
						startRetryTimer();
					}
				} finally {
